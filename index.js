...
async function searchForMovies(){
    ...
    const response = await fetch(`https://cors-anywhere.herokuapp.com/http://www.omdbapi.com/?apikey=${myAPIkey}&s=${text}`);
    const data = await response.json();
    movieDiv.innerHTML = ``;
    
    if(data.Search){
        for(let i=0; i<data.Search.length; i++){
            const res = await fetch(`https://cors-anywhere.herokuapp.com/http://www.omdbapi.com/?apikey=${myAPIkey}&i=${data.Search[i].imdbID}`);
